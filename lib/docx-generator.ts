...
export const generateDOCX = async (resume: ResumeState["resume"]) => {
  try {
...
${resume.socialLinks.map((link) => `• ${link.platform}: ${link.url}`).join("\n")}
    ...
    // Create a blob that simulates a DOCX file
    const blob = new Blob([content], {
      type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    })

    // Save the blob as a file
    FileSaver.saveAs(blob, `${resume.personalInfo.firstName}_${resume.personalInfo.lastName}_Resume.docx`)
